...
local_regression <- function(ID_individual, data = data, p=p, longlat=longlat,
                             bw=bw, kernel=kernel, adaptive=adaptive, index = index,
                             model=model, effect = effect, random.method = random.method)
{
  data$aim[data$id == ID_individual] <- 1
  data$aim[data$id != ID_individual] <- 0
  subsample <- data
                           ...
                           random.method = random.method, cluster.number = cluster.number)
{
  ...
  residualsVector <- parallel::mclapply(ID_list_single, FUN = local_regression,
                                        data = data, p=p, longlat=longlat,
                                        bw=bw, kernel=kernel, adaptive=adaptive, 
                                        index = index, model=model, effect = effect,
                                        random.method = random.method, mc.cores = cluster.number)
  residualsVector <- unlist(residualsVector)
  CVscore <- nrow(data) * sum(residualsVector^2) /
    (nrow(data) - length(varibale_name_in_equation) + 1)^2
  cat("Fixed Bandwidth:", bw, "CV score:", CVscore, "\n")
  return(CVscore)
